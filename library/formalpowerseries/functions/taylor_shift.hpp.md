...
data:
  ...
  bundledCode: "#line 1 \"library/util/Valarray.hpp\"\n#include <functional>\n#include\
    ...
    \    res[0] = 1;\n    for (int i = 1; i < MX; i++) {\n        res[i] = res[i -\
    \ 1] * n / i;\n    }\n    return res;\n}\n\n} // namespace fps\n#line 4 \"library/formalpowerseries/functions/taylor_shift.hpp\"\
    \n#include <algorithm>\n\nnamespace fps {\n\ntemplate <typename T, int MX>\nvoid\
    \ taylor_shift(FormalPowerSeries<T, MX>& f, T c) {\n    f.shrink();\n    if (f.size()\
    \ <= 1 || c == 0) {\n        return;\n    }\n    int n = f.size();\n    T fact\
    \ = 1;\n    for (int i = 0; i < n; i++) {\n        if (i) {\n            fact\
    \ *= i;\n        }\n        f[i] *= fact;\n    }\n    std::reverse(f.begin(),\
    \ f.end());\n    f *= exp<T, MX>(c).pre(n);\n    f.strict(n);\n    std::reverse(f.begin(),\
    ...
    \ fps\n"
  code: "#pragma once\n#include \"../Base.hpp\"\n#include \"./exp.hpp\"\n#include\
    \ <algorithm>\n\nnamespace fps {\n\ntemplate <typename T, int MX>\nvoid taylor_shift(FormalPowerSeries<T,\
    \ MX>& f, T c) {\n    f.shrink();\n    if (f.size() <= 1 || c == 0) {\n      \
    \  return;\n    }\n    int n = f.size();\n    T fact = 1;\n    for (int i = 0;\
    \ i < n; i++) {\n        if (i) {\n            fact *= i;\n        }\n       \
...
layout: document
redirect_from:
- /library/library/formalpowerseries/functions/taylor_shift.hpp
- /library/library/formalpowerseries/functions/taylor_shift.hpp.html
